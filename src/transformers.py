...
class DataTransformer:
    ...
    @staticmethod
    def clean_nulls(
        df: DataFrame, strategy: str = "drop", fill_value: dict[str, Any] | None = None
    ) -> DataFrame:
    ...
    def add_derived_column(
        df: DataFrame, column_name: str, expression: str
    ) -> DataFrame:
    ...
    def normalize_column(
        df: DataFrame, column: str, new_column: str | None = None
    ) -> DataFrame:
        ...
        if std_val and std_val > 0:
            df = df.withColumn(col_name, (F.col(column) - mean_val) / std_val)
        return df

    @staticmethod
    def bucket_column(
        df: DataFrame, column: str, num_buckets: int, new_column: str = "bucket"
    ) -> DataFrame:
        ...
        """
        return df.withColumn(new_column, F.floor(F.col(column) * num_buckets))

    @staticmethod
    def parse_json_column(
        df: DataFrame, column: str, schema: Any | None = None
    ) -> DataFrame:
        ...
        """
        return (
            df.withColumn("parsed", F.from_json(F.col(column), schema or "string"))
            .withColumn(column, F.col("parsed"))
            .drop("parsed")
        )


def apply_transformation_pipeline(
    df: DataFrame, transformations: list[dict[str, Any]]
) -> DataFrame:
